# BeatAPI SaaS Template — Agent Instructions

This folder is the **SaaS Template** (auth, payments, credits, admin). The open-source workbench lives next to it at `../BeatAPI Workspace`. Do not mix workspace generation work back into this template.

This is the isolated **BeatAPI SaaS Template** staging repository. Never edit the source repository while working here. Preserve the reusable product infrastructure (payments, credits, subscriptions, auth, RBAC) and treat product-specific branding and workflows as extraction candidates until the template boundary is finalized.
...
- **Database:** Drizzle ORM with PostgreSQL in production. SQLite exists only as a local test fixture.
...
```bash
pnpm install              # install dependencies (postinstall selects the configured schema)
...
│   ├── db/                      # PostgreSQL and local SQLite fixture
...
Key entry files:
- `vite.config.ts` — plugins: mdx → tailwindcss → **paraglideVitePlugin** (outdir `./src/paraglide`, `strategy: ['url','cookie','baseLocale']`, `urlPatterns` with English unprefixed and Chinese under `/zh`) → tanstackStart → viteReact → nitro. Calls `loadEnvFiles()` so server-side `process.env` is populated from `.env.{NODE_ENV}`.
- `src/server.ts` — Nitro fetch entry; wraps the request in `paraglideMiddleware` so `getLocale()` resolves server-side.
- `src/router.tsx` — exports `getRouter`; its `rewrite` runs Paraglide `deLocalizeUrl`/`localizeUrl` so routes stay locale-free while Chinese URLs gain the `/zh` prefix.
...
**Message source:** flat and nested JSON in `messages/en.json` and `messages/zh.json` (~1350 leaf keys). English is the base locale and stays unprefixed; Chinese lives under `/zh`. The key prefix is the old namespace, e.g. `"landing.hero.headline"`, `"common.nav.profile"`, `"admin.users.credits_granted"`. The Inlang project config lives at `project.inlang/settings.json`. The compiler output `src/paraglide/` is **gitignored** and regenerated by the vite plugin on each build/dev.
...
The `/quick-start` skill automates this original product workflow.
...
| Skill | When to use |
|-------|-------------|
| `quick-start` | Build an original SaaS project from a product brief |
...
| `deploy-cloudflare` | Deploy to Cloudflare Workers with PostgreSQL, Hyperdrive, secrets, and migrations |

**Database on Cloudflare Workers:** use PostgreSQL through a `HYPERDRIVE`
binding. `src/core/db/postgres.ts` reads the binding connection string from the
Workers environment. Schema, RBAC, and admin scripts use the direct
`DATABASE_URL`; they do not run migrations through Hyperdrive. SQLite remains a
local test fixture and is not a production billing database.
...
Two dialect templates are committed to git:
- `src/config/db/schema.postgres.ts` — Supabase/PostgreSQL (default)
- `src/config/db/schema.sqlite.ts` — local SQLite fixture only
...
# App (optional, public)
VITE_APP_DESCRIPTION=Open SaaS starter for AI products
VITE_APP_LOGO=
VITE_DEFAULT_LOCALE=en
VITE_GOOGLE_AUTH_ENABLED=false
VITE_GOOGLE_ONE_TAP_ENABLED=false
...
**Provider credentials can live in the admin panel or server env.** Payments
(Stripe/Creem/Lemon Squeezy/Paddle/PayPal and the China add-on), OAuth
(Google/One-Tap), email (Resend/Mailgun), storage (R2), AI, and analytics are configured at
...
Keep `.env.example` synchronized with the public support matrix.
