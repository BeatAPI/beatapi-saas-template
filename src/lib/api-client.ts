...
async function request<T>(url: string, init?: RequestInit): Promise<T> {
  const res = await apiRequestRaw(url, init);
...
async function requestJson<T>(
  ...
  if (!res.ok) {
    throw new ApiError(
      res.status,
      json?.error ?? json?.message ?? fallback,
      json
    );
  }
...
export const apiFormPost = <T = void>(url: string, body: FormData) =>
  request<T>(url, { method: 'POST', body });

export const apiRequestRaw = (url: string, init?: RequestInit) => {
  const isFormData =
...
export const apiJsonGet = <T>(url: string, init?: RequestInit) =>
  requestJson<T>(url, init);

export const apiJsonPost = <T = void>(
  url: string,
  body?: unknown,
  init?: RequestInit
) =>
...
export const apiJsonPut = <T = void>(
  url: string,
  body?: unknown,
  init?: RequestInit
) =>
...
export const apiJsonPatch = <T = void>(
  url: string,
  body?: unknown,
  init?: RequestInit
) =>
...
export const apiJsonDelete = <T = void>(
  url: string,
  body?: unknown,
  init?: RequestInit
) =>
