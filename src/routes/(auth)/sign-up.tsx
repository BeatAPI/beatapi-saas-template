...
function SignUpPage() {
  ...
  const signInHref =
    afterLoginUrl === '/'
      ? '/sign-in'
      : `/sign-in?callbackUrl=${encodeURIComponent(afterLoginUrl)}`;
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        if (inviteCodeRequired) {
          try {
            await apiPost("/api/invite-codes/validate", { code: trimmedInvite });
          } catch (err: any) {
        ...
        setError(
          err instanceof VerificationEmailDeliveryError
            ...
            : err.message || m["common.sign.sign_up_failed"]()
        );
      }
    },
  });
  ...
  return (
    <AuthPageShell title={m["common.sign.sign_up_title"]()}>
                    ...
                    <form.Field name="confirmPassword">
                      {(field) => (
                        <TextField
                          ...
                          type="password"
                          required
                          placeholder={m["common.sign.confirm_password_placeholder"]()}
                        />
                      )}
                    </form.Field>
                    {inviteCodeRequired && (
                      ...
                      <form.Subscribe selector={(s) => s.isSubmitting}>
                        {(isSubmitting) => (
                          <Button type="submit" disabled={isSubmitting}>
                            {isSubmitting ? "..." : m["common.sign.sign_up_title"]()}
                          </Button>
                        )}
                      </form.Subscribe>
